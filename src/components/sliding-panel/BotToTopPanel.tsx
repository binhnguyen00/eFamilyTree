...
export function BotToTopPanel(props: PanelProps) {
  const { children, height = 450, visible, header, close, className } = props;
  ...
  return ReactDOM.createPortal(
    <>
      ...
      <animated.div
        ...
        className={`rounded-top bg-white ${className ? className : ""}`}
      >
        <div className="text-primary">
          <HeaderSection header={header} close={reverseOpen}/>
          <div className={`p-2 flex-v ${className ? className : ""}`}>
            {children}
          </div>
        </div>
      </animated.div>
...
function HeaderSection(props: HeaderSectionProps) {
  return (
    <div
      ...
      className="bg-primary text-secondary p-2 rounded-top"
    >
